 ...
 * @author bx002
 */
public class PixivPostTemplate {
    public static final String URL_ARTWORK_PREFIX = "https://www.pixiv.net/artworks/";

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PixivPostTemplate.class);
    /**
     * 作品详情接口(cookie可选)
     */
    ...
    public static List<JSONObject> getBookmarks(String uid, String cookie, String tag, Integer page, ThreadPoolTaskExecutor executor, Map<String, Integer> progressMap) {
        ...
        if (offset < total) {
            ...
            while (offset < total) {
                ...
                tasks.add(() -> {
                    JSONObject bookmarks = PixivPostTemplate.getBookmarks(uid, cookie, tag, limit, finalOffset);
...
//                .getResult();
                ...
                GetRequest.create()
                        ...
                        .addParam("mode", mode)
                        .addParam("p", p)
                        .get(URL_ILLUST_SEARCH + URLEncoder.encode(keyword, StandardCharsets.UTF_8));
